...
show_help() {
    ...
    echo "远程使用："
    echo "  # 一键自动备份"
    echo "  curl -fsSL https://raw.githubusercontent.com/moli-xia/docker-backup-tool/main/docker_backup_all_in_one.sh | bash"
    echo ""
    echo "  # 安装到本地使用"
    echo "  curl -fsSL https://raw.githubusercontent.com/moli-xia/docker-backup-tool/main/docker_backup_all_in_one.sh | bash -s -- --install"
    echo ""
    echo "备份内容："
    echo "  • 容器配置和环境变量"
    echo "  • Docker数据卷"
...
main_menu() {
    # 检查是否有交互式终端
    if ! [ -t 0 ]; then
        print_error "无法在非交互式环境中运行菜单"
        print_info "请使用: $0 --auto (自动备份) 或 $0 --help (查看帮助)"
        exit 1
    fi
...
install_to_local() {
    ...
    if ! mkdir -p "$install_dir" 2>/dev/null; then
        print_warning "无法创建 $install_dir，尝试当前目录"
        install_dir="$(pwd)"
    fi
    ...
    if command -v curl >/dev/null 2>&1; then
        curl -fsSL "https://raw.githubusercontent.com/moli-xia/docker-backup-tool/main/$script_name" -o "$script_path"
    elif command -v wget >/dev/null 2>&1; then
        wget -q "https://raw.githubusercontent.com/moli-xia/docker-backup-tool/main/$script_name" -O "$script_path"
    else
    ...
    if [ -f "$script_path" ]; then
        ...
        echo ""
        print_info "使用方法："
        echo "  $script_path                    # 交互式界面"
        echo "  $script_path --auto             # 自动备份"
        echo "  $script_path --help             # 查看帮助"
        
        # 询问是否立即运行
        if [ -t 0 ]; then  # 只有在有终端输入时才询问
            echo ""
            read -p "是否现在启动交互式界面？(y/N): " -r reply
            if [[ $reply =~ ^[Yy]$ ]]; then
                exec "$script_path"
            fi
        fi
...
# 主程序入口
main() {
    # 检查是否从管道执行（如 curl | bash）
    if is_piped && [ $# -eq 0 ]; then
        print_warning "检测到通过管道执行，自动切换到自动备份模式"
        print_info "如需交互式界面，请下载脚本到本地执行"
        echo ""
        auto_backup
        return
    fi
